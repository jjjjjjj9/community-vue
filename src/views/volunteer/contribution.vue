<template>
    <div>
      <el-card class="box-card box-cards">
        <el-row>
          <el-form :inline="true" class="user-search">
            <el-col :span="24">
              <el-form-item>
                <div class="card_top"><span>爱心款管理</span></div>
              </el-form-item>
      ...
      <el-card class="box-card box-cards2">
        ...
        <el-table border ref="multipleTable" :data="tableData" :header-cell-style="headStyle" tooltip-effect="dark" style="width: 100%; "
          ...
          <el-table-column prop="sex" label="是否匿名" align="center"  width="120">
          <template slot-scope="scope">
            <div>{{ scope.row.ni == 1 ? "是" : "否"  }}</div>
          </template>
        </el-table-column>
          <el-table-column prop="pay" label="捐款金额" align="center"  width="120"></el-table-column>
          <el-table-column prop="logo" label="支付凭证" align="center" width="120">
              <template slot-scope="scope">
  ...
  import { ContributionList,Contributionshenhe, ContributionDel } from "../../api/userMG";
  export default {
    data() {
      return {
        optionslist: [{
    ...
    methods: {
      ...
      handleEdit(index, row,e) {
        this.$confirm("确定要更变校验类型吗?", "信息", {
          ...
          .then(() => {
            Contributionshenhe({ id: row.id,status:e }) //删除
              .then((res) => {
                // console.log("d", res);
                if (res.code == 200) {
                  this.$message({
