    ...
    <div>
        <el-card class="box-card box-cards">
            <el-row>
                <el-form :inline="true" class="user-search">
                    <el-col :span="24">
                        <el-form-item>
                            <div class="card_top"><span>业主投诉建议</span></div>
                        </el-form-item>
        ...
        <el-card class="box-card box-cards2">
            ...
            <el-table border ref="multipleTable" :data="tableData" :header-cell-style="headStyle" tooltip-effect="dark"
                ...
                <el-table-column prop="atitle" label="标题" align="center" width="300"></el-table-column>

                <el-table-column prop="lx" label="求助类型" align="center" width="120">
                    <template slot-scope="scope">
                    <div>{{ scope.row.lx==1?"建议":"投诉"}}</div>
                    </template>
                </el-table-column>
                
                <el-table-column prop="details" label="内容描述" align="center" width="300"></el-table-column>
                <el-table-column prop="logo" label="相关图片" align="center" width="280">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="phone" label="处理状态" align="center" width="120" >
                    <template slot-scope="scope">
                    <div v-if="scope.row.status == 0">待接收</div>
                    <div v-if="scope.row.status == 1">已接收</div>
                    <div v-if="scope.row.status == 2">已处理</div>

                    </template>
                </el-table-column>
                <el-table-column prop="aname" label="回复内容" align="center" width="160"></el-table-column>
                <el-table-column align="center" label="操作" width="220" fixed="right">
                    <template slot-scope="scope">
                        <el-button size="mini" type="success"  @click="handleEdit(scope.$index, scope.row)"  v-if="scope.row.status ==0" >接收</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <!-- 分页组件 -->
        ...
        <!-- 新增 -->
        <el-drawer title="投诉意见" :visible.sync="editFormVisible" :before-close="handleClose" direction="rtl"
                    ...
                    <el-form label-width="110px" :model="editForm" :rules="rules" ref="editForm">
                        ...
                        <el-col :span="22">
                            <div class="grid-content bg-purple">
                                <el-form-item label="选择师傅：" prop="enable"  v-if="editForm.lx==0">
                                    <el-select v-model="editForm.aid" placeholder="请选择师傅">
                                        <el-option v-for="item in useroptions" :key="item.id" :label="item.name" :value="item.id">
                                        </el-option>
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="回复内容：" prop="unit">
                                    <el-input v-model="editForm.aname"  placeholder="请输入回复内容"></el-input>
                                </el-form-item>
...
import { SuggestionAp, SuggestionList,weiuserlist} from "../../api/userMG";
export default {
    data() {
        return {
            options: [],
